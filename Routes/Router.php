...
class Router {
    ...
    public function get (string $path, $callback, ?string $name): self
    {
        ...
        if (!is_null($name)) {
            ...
            if (isset($this->nameRoutes[$name])) {
               throw new Exception("Il y a une route qui porte ce nom ");
            }
    ...
    public function run ()
    {
        if(!isset($this->routes[$_SERVER['REQUEST_METHOD']])){
            throw new Exception("Aucune methode n'est définie");
        }
        ...
        throw new Exception('La route que vous avez pris n\'existe pas ');
    }
}